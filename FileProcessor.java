...
public class FileProcessor {
	...
	public void processfile(BufferedReader input, BufferedReader customer, BufferedReader order)
		...
		while ((line = input.readLine()) != null) {
			...
			} else if (splittedstring[0].equals("List")) {
				out.println("Customer List:");
				mngCust.sortByName();
				for (Customer cs : mngCust.getAllCustomers()) {
					out.println(cs.getCustomerID() + " " + cs.getCustomerName() + " " + cs.getCustomerPhone() + " Address: " + cs.getCustomerAddress());
				}
			} else {
		...
		mngCust.sortByID();
		for (Customer cs : mngCust.getAllCustomers()) {
			writeCustomers.println(cs.getCustomerID() + " " + cs.getCustomerName() + " " + cs.getCustomerPhone() + " Address: " + cs.getCustomerAddress());
		}
		mngCust.sortByID();
